...
`POST /api/v1/auth/logout` завершує поточну refresh-сесію. Якщо cookie містить
активний token, `RefreshTokenServiceImpl` відкликає саме його. Потім
`RefreshTokenCookieFactory` повертає cookie з порожнім значенням і
`Max-Age=0`, щоб клієнт видалив його.

Logout ідемпотентний: відсутній, невідомий або вже відкликаний token не дає
помилки, а endpoint усе одно повертає `204`. Уже виданий access JWT не
відкликається миттєво й залишається чинним до завершення `exp`.
